...
class Visualizaciones:
    ...
    def distribucion_bajas_doble(
        ...
        - columna_1 → gráfico de barras
        - columna_2 → gráfico de pie
        """
        ...
        if titulos is not None:
            ...
            titulo_1 = f"Distribution of Leavers by {columna_1.replace('_', ' ')} (%)"
        ...
        # 2. PIE CHART (Department)

        if titulos is not None:
            titulo_2 = titulos[1]
        else:
            titulo_2 = f"Distribution of Leavers by {columna_2.replace('_', ' ')} (%)"

        colores = sns.color_palette(self.palette_seq, len(serie_2))

        axes[1].pie(
            serie_2.values,
            labels=serie_2.index,
            autopct="%.1f%%",
            startangle=90,
            colors=colores)

        axes[1].set_title(titulo_2, fontsize=14, fontweight="bold")
